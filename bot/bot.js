...
//            });
            ...
            try {
                var execute = commands[commandInfo[0].toLowerCase()].process(bot, SQLconn, user, userID, channelID, message, rawEvent, replyID, commandInfo);
            } catch(error) {
                if (typeof commands[commandInfo[0]] == 'undefined') {
                    console.log("Command '" + commandInfo[0] + "' not found!");
                    bot.sendMessage({
                        to: channelID,
                        message: replyID + " bruh, I think you typed the command name in wrong; I can't seem to find it anywhere. And trust me, I've tried. I searched high, and I searched low, I even found Warwick Davis on my travels! But alas, thy labour hath been fruitless. To get a list of commands (and thus educate thyself), type `>commands`.\n\nPerhaps you thought I had this command, or you think I _should_ have this command. If so, then you can submit a sugguestion to _The Creator_ through the command `>suggest(<suggestion>)`."
                    });
                } else {
                    console.log("Error while executing " + commandInfo[0].toUpperCase() + ": " + error);
                    
                    var err;
                    if (commands[commandInfo[0].toLowerCase()].functional = false) {
                        err = replyID + " Sorry m8, some sort of error was encountered while trying to execute the command... `" + error + "`\n Please contact my Creator to make them aware of this issue.";
                    } else {
                        err = replyID + " Sorry m8, some sort of error was encountered while trying to execute the command... `" + error + "`\nPlease keep in mind that this command is marked as `non-functional`, and does not appear in any command lists. My Creator will be aware of this issue, so you'll just have to cry my network port a river in the meantime :neutral_face:";
                    }
                    bot.sendMessage({
                        to: channelID,
                        message: err
